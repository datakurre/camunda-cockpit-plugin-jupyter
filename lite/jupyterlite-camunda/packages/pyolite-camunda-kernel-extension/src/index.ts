 ...
 */
const PLUGIN_ID = '@jupyterlite/pyolite-camunda-kernel-extension:kernel';
  ...
  activate: (app: JupyterLiteServer, kernelspecs: IKernelSpecs) => {
    ...
    kernelspecs.register({
      ...
      create: async (options: IKernel.IOptions): Promise<IKernel> => {
        const { PyoliteCamundaKernel } = await import('@jupyterlite/pyolite-camunda-kernel');

        return new PyoliteCamundaKernel({
          ...options,
          pyodideUrl,
          pipliteUrls,
          disablePyPIFallback,
